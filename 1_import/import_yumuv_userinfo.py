...
df_initial_survey = pd.read_sql(
    """select distinct on(externalreference) * from 
henry_dev.eth_survey order by externalreference, progress desc""",
    con=engine,
    index_col="externalreference",
)

df_final_survey_kg = pd.read_sql(
    """select distinct on(externalreference) * from 
henry_dev.eth_final_survey_kg order by externalreference, progress desc""",
    con=engine,
    index_col="externalreference",
)

df_final_survey_fk = pd.read_sql(
    """select distinct on(externalreference) * from 
henry_dev.eth_final_survey_fk order by externalreference, progress desc""",
    con=engine,
    index_col="externalreference",
)
...
initial_drop = [
    ...
    "metadata_resolution",
    "participation_timing_first click",
    "participation_timing_last click",
    "participation_timing_page submit",
    "participation_timing_click count",
    "q221",
    "q222",
    "q223_first click",
    "q223_last click",
    ...
    "q745",
    "q746",
    "hh_timing_p1_first click",
    "hh_timing_p1_last click",
    "hh_timing_p1_page submit",
    "hh_timing_p1_click count",
    "q228_1",
    "q228_2",
    "q228_3",
    "q228_4",
...
df_user_id = pd.read_sql(
    """select user_id, study_id, survey_code as externalreference from henry_dev.user_dates""",
    con=engine,
    index_col="externalreference",
)
