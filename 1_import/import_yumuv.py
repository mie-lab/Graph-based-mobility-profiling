...
tpls = ti.io.read_triplegs_gpd(tpls, user_id="story_line_uuid", geom_col="geom", tz="UTC")
...
ti.io.write_staypoints_postgis(
    staypoints=sp, con=engine, name="staypoints", schema="yumuv_graph_rep", if_exists="replace",
)

print("write triplegs")

ti.io.write_triplegs_postgis(
    triplegs=tpls, con=engine, name="triplegs", schema="yumuv_graph_rep", if_exists="replace",
)
...
ti.io.write_locations_postgis(locs, name="locations", con=engine, schema="yumuv_graph_rep", if_exists="replace")
