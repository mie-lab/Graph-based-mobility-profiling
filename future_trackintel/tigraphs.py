...
def weights_transition_count(staypoints, adjacency_dict=None):
    ...
    # todo: check gaps between staypoints
    # todo: create directed edges
    ...
    # todo:  we can exclude gaps by adding a dummy staypoint with no loc id in between the locations
    # #todo: or we go via trips.
    ...
    adjacency_dict = _create_adjacency_matrix_from_counts(counts, all_users, adjacency_dict)

    return adjacency_dict


def _create_adjacency_matrix_from_counts(counts, user_list, adjacency_dict=None):
    """
    ...
    for user_id_this in user_list:
        ...
        values = counts_user["counts"].values.astype("float")
...
def generate_activity_graphs(locations, adjacency_dict, node_feature_names=[]):
    ...
    for user_id_this in locations["user_id"].unique():
        ...
        locs_user_view = locations.loc[locations.index == user_id_this]
        ...
        for ix in range(len(A_list)):
            A = A_list[ix]
            # todo: do I really need to create a dense matrix?
            ...
            # Todo: G should be directed by default. A is directed but G.is_directed() returns false
...
def weights_delaunay(locations, to_crs=None, distance_matrix_metric="haversine", adjacency_dict=None):
    ...
    for user_id_this in all_users:
        ...
        if user_id_this not in adjacency_dict:
            adjacency_dict[user_id_this] = {"A": [A], "loc_id_order": [loc_id_order], "edge_name": [edge_name]}
        else:
            adjacency_dict[user_id_this]["A"].append(A)
...
def count_locations_by_user(staypoints):
    ...
    for user_id in all_users:
        unique_locations = staypoints_a.loc[staypoints_a["user_id"] == user_id, "cluster_id"].unique()
